...
check_http_endpoint() {
    ...
    local url="https://${app_name}.hackthacker.lab${uri}"
    ...
    local code=$(curl -s -k -o /dev/null -w "%{http_code}" -H "Host: ${app_name}.hackthacker.lab" "https://127.0.0.1:443" --connect-timeout 2)
...
while true; do
    ...
    case "$opt" in
        ...
        3)
            ...
            echo -e "  ${GRN}${SERVER_IP}${RST}  ${YLW}${APPS_LIST[*]/%/.hackthacker.lab}${RST}"
            ;;
        4)
